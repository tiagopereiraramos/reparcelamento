...
class TesteSiengeRobusto:
    ...
    async def inicializar_sistema(self) -> bool:
        ...
        try:
            ...
            if not self.credenciais_teste.get("usuario"):
                self._log_passo(
                    "Credenciais não configuradas - modo simulação", "AVISO")
                return False
            else:
    ...
    async def teste_1_validacao_estrutura_sistema(self) -> bool:
        ...
        try:
            ...
            metodos_validacao = [
                "validar_cliente", "classificar_inadimplencia"
            ]
            ...
            self._log_passo("TESTE 1 CONCLUÍDO",
                            "SUCESSO" if sucesso_total else "FALHA")
    ...
    async def teste_2_processamento_regras_pdd(self) -> bool:
        ...
        try:
            ...
            planilha_real = Path(
                "attached_assets/saldo_devedor_presente-20250610-093716.xlsx")

            if not planilha_real.exists():
                self._log_passo("Planilha real não encontrada nos assets",
                                "FALHA")
                return False

            df_real = pd.read_excel(planilha_real)
            self._log_passo(f"Planilha carregada: {len(df_real)} registros",
                            "SUCESSO")
            ...
            self._log_passo(
                f"Status Cliente: {resultado_validacao.get('status_cliente')}",
                "SUCESSO")
            self._log_passo(
                f"Pode Reparcelar: {resultado_validacao.get('pode_reparcelar')}",
                "SUCESSO")
            self._log_passo(
                f"CT Vencidas: {resultado_validacao.get('qtd_ct_vencidas', 0)}",
                "SUCESSO")
            ...
            if resultado_regras:
                ...
                self._log_passo(
                    f"Dia vencimento: {resultado_regras.get('dia_vencimento')}",
                    "SUCESSO")
                self._log_passo(
                    f"Valor parcela atual: R$ {resultado_regras.get('valor_parcela_atual', 0):,.2f}",
                    "SUCESSO")
                self._log_passo(
                    f"1º vencimento carnê: {resultado_regras.get('primeiro_vencimento_carne')}",
                    "SUCESSO")
            ...
            if calculo_resultado.get("sucesso"):
                ...
                self._log_passo(
                    f"Novo saldo calculado: R$ {calculo_resultado.get('novo_saldo', 0):,.2f}",
                    "SUCESSO")
                self._log_passo(
                    f"Fator correção: {calculo_resultado.get('fator_correcao', 1):.4f}",
                    "SUCESSO")
            else:
                self._log_passo(
            ...
            self._log_passo(
                f"Parcelas para desmarcar: {len(parcelas_desmarcar)}",
                "SUCESSO")
    ...
    async def teste_3_integracao_data_manager(self) -> bool:
        ...
        try:
            ...
            self._log_passo(
                "Testando carregamento dados fila reparcelamento...")
            try:
                resultado_carga = await self.rpa_sienge.carregar_dados_fila_reparcelamento(
                )
                resultados["carregamento_fila"] = resultado_carga.get(
                    "sucesso", False)

                if resultado_carga.get("sucesso"):
                    self._log_passo("Dados da fila carregados com sucesso",
                                    "SUCESSO")
                    parametros = resultado_carga.get("parametros_navegacao",
                                                     {})
                    self._log_passo(
                        f"Título carregado: {parametros.get('numero_titulo', 'N/A')}",
                        "SUCESSO")
                elif resultado_carga.get("fila_vazia"):
                    self._log_passo("Fila de reparcelamento vazia", "AVISO")
                else:
                    self._log_passo(
                        f"Erro no carregamento: {resultado_carga.get('erro')}",
                        "FALHA")
    ...
    async def teste_4_login_sienge(self) -> bool:
        ...
        try:
            ...
            if not self.credenciais_teste.get("usuario"):
                self._log_passo(
                    "Credenciais não configuradas - simulando login", "AVISO")
            ...
            self.resultados_teste["teste_4_login"] = resultados
            return resultados.get("login_sucesso", False) or resultados.get(
                "login_simulado", False)
    ...
    async def teste_5_consulta_relatorios(self) -> bool:
        ...
        try:
            ...
            if not self.rpa_sienge.logado_sienge:
                self._log_passo("Sistema não logado - simulando consulta",
                                "AVISO")
                ...
                if planilha_real.exists():
                    ...
                    resultado_processamento = await self.rpa_sienge._processar_planilha_baixada(
                        contrato_teste["cliente"],
                        contrato_teste["numero_titulo"])

                    resultados["consulta_simulada"] = True
                    resultados[
                        "processamento_planilha"] = resultado_processamento.get(
                            "sucesso", False)

                    if resultado_processamento.get("sucesso"):
                        self._log_passo(
                            "Processamento planilha simulado com sucesso",
                            "SUCESSO")
                        dados_validacao = resultado_processamento.get(
                            "dados_validacao", {})
                        self._log_passo(
                            f"Status: {dados_validacao.get('status_cliente')}",
                            "SUCESSO")
                    else:
                        self._log_passo(
                            f"Erro no processamento: {resultado_processamento.get('erro')}",
                            "FALHA")
                else:
                    self._log_passo(
                        "Planilha real não encontrada para simulação", "FALHA")
                    resultados["consulta_simulada"] = False
            else:
                # Consulta real no Sienge
                self._log_passo("Executando consulta real no Sienge...")
                try:
                    dados_financeiros = await self.rpa_sienge._consultar_relatorios_financeiros(
                        contrato_teste)

                    resultados["consulta_real"] = True
                    resultados["sucesso_consulta"] = dados_financeiros.get(
                        "sucesso", False)

                    if dados_financeiros.get("sucesso"):
                        self._log_passo("Consulta realizada com sucesso",
                                        "SUCESSO")
                        self._log_passo(
                            f"Cliente: {dados_financeiros.get('cliente')}",
                            "SUCESSO")
                        self._log_passo(
                            f"Arquivo gerado: {dados_financeiros.get('arquivo_processado')}",
                            "SUCESSO")
                    else:
                        self._log_passo(
                            f"Erro na consulta: {dados_financeiros.get('erro')}",
                            "FALHA")

                except Exception as e:
                    resultados["consulta_real"] = False
                    resultados["erro_consulta"] = str(e)
                    self._log_passo(f"Erro na consulta real: {str(e)}",
                                    "FALHA")
    ...
    async def teste_6_execucao_completa(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self._log_passo(
                    f"Execução consulta: {resultado_consulta.sucesso}", status)

                if not resultado_consulta.sucesso:
                    self._log_passo(f"Erro: {resultado_consulta.erro}",
                                    "FALHA")

            except Exception as e:
                resultados["execucao_consulta"] = False
                self._log_passo(f"Erro na execução consulta: {str(e)}",
                                "FALHA")
            ...
            try:
                resultado_reparcelamento = await self.rpa_sienge.executar(
                    ...
                    True,  # Força autorização para teste
                    notificar_analista=False)

                resultados[
                    "execucao_reparcelamento"] = resultado_reparcelamento.sucesso
                status = "SUCESSO" if resultado_reparcelamento.sucesso else "FALHA"
                self._log_passo(
                    f"Execução reparcelamento: {resultado_reparcelamento.sucesso}",
                    status)

                if not resultado_reparcelamento.sucesso:
                    self._log_passo(f"Erro: {resultado_reparcelamento.erro}",
                                    "FALHA")

            except Exception as e:
                resultados["execucao_reparcelamento"] = False
                self._log_passo(f"Erro na execução reparcelamento: {str(e)}",
                                "FALHA")
    ...
    async def teste_7_webscraping_producao(self) -> bool:
        ...
        try:
            ...
            if not self.credenciais_teste.get("usuario"):
                self._log_passo(
                    "Simulando teste de webscraping (sem credenciais reais)",
                    "AVISO")
                ...
                try:
                    ...
                    resultados[
                        "sucesso_webscraping"] = resultado_webscraping.sucesso

                    if resultado_webscraping.sucesso:
                        self._log_passo("Webscraping executado com sucesso",
                                        "SUCESSO")
                        dados = resultado_webscraping.dados
                        if dados:
                            self._log_passo(
                                f"Título: {dados.get('numero_titulo')}",
                                "SUCESSO")
                            self._log_passo(
                                f"Novo título: {dados.get('novo_titulo_gerado')}",
                                "SUCESSO")
                    else:
                        self._log_passo(
                            f"Erro no webscraping: {resultado_webscraping.erro}",
                            "FALHA")

                except Exception as e:
                    resultados["webscraping_real"] = False
                    self._log_passo(f"Erro no webscraping real: {str(e)}",
                                    "FALHA")
    ...
    def _salvar_resultados_teste(self):
        ...
        try:
            ...
            dados_completos = {
                ...
                bool(self.credenciais_teste.get("usuario")),
                "resultados_detalhados":
                self.resultados_teste,
                "resumo_testes":
                self._gerar_resumo_testes(),
                "ambiente_teste": {
    ...
    def _gerar_resumo_testes(self) -> Dict[str, Any]:
            ...
            if isinstance(resultado, dict):
                ...
                if valores:
                    ...
                    resumo["detalhes_por_teste"][nome_teste] = {
                        ...
                        "total_verificacoes":
                        len(valores),
                        "verificacoes_bem_sucedidas":
                        sum(1 for v in valores if v is True)
                    }
        ...
        if resumo["total_testes_executados"] > 0:
            resumo["taxa_sucesso_percentual"] = (
                resumo["testes_bem_sucedidos"] /
                resumo["total_testes_executados"]) * 100
    ...
    async def executar_bateria_completa(self) -> bool:
        ...
        try:
            ...
            testes_bem_sucedidos = sum(1 for _, sucesso in testes_executados
                                       if sucesso)
            ...
            for nome_teste, sucesso in testes_executados:
                ...
                self.logger.info(
                    f"{emoji} {nome_teste}: {'PASSOU' if sucesso else 'FALHOU'}"
                )

            self.logger.info(f"\n📈 ESTATÍSTICAS FINAIS:")
            self.logger.info(
                f"   ✅ Testes bem-sucedidos: {testes_bem_sucedidos}/{total_testes}"
            )
