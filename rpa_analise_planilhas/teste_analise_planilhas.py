...
async def teste_regras_pdd_csv_completo():
    ...
    try:
        ...
        if not os.path.exists(dados_teste_csv["arquivo_csv"]):
            print(f"❌ Arquivo CSV não encontrado: {dados_teste_csv['arquivo_csv']}")
            return False
        ...
        df_titulo = df_csv[df_csv["Título"] == int(dados_teste_csv["numero_titulo"])]
        print(f"🎯 Registros do título {dados_teste_csv['numero_titulo']}: {len(df_titulo)}")

        if df_titulo.empty:
            print(f"❌ Nenhum registro encontrado para o título {dados_teste_csv['numero_titulo']}")
            return False
        ...
        tipos_documento = df_titulo["Documento"].value_counts()
        ...
        print(f"🎯 Sucesso: {'✅ SIM' if resultado_pdd.get('sucesso') else '❌ NÃO'}")
        ...
        print(f"   Pode Reparcelar: {'✅ SIM' if resultado_pdd.get('pode_reparcelar') else '❌ NÃO'}")
        ...
        print(f"   💰 Valor Parcela Atual: R$ {resultado_pdd.get('valor_parcela_atual', 0):,.2f}")
        print(f"   📆 Primeiro Vencimento: {resultado_pdd.get('primeiro_vencimento_carne')}")
        print(f"   📊 Parcelas CT a Vencer: {resultado_pdd.get('qtd_parcelas_ct_a_vencer')}")
        print(f"   🏠 Parcelas IPTU a Vencer: {resultado_pdd.get('qtd_parcelas_iptu_a_vencer')}")
        print(f"   💵 Saldo Total: R$ {resultado_pdd.get('saldo_total', 0):,.2f}")

        # IRREGULARIDADES
        if resultado_pdd.get("tem_parcelas_irregulares"):
            print(f"   ⚠️ Parcelas Irregulares: {resultado_pdd.get('quantidade_irregulares')}")
        else:
            print(f"   ✅ Sem Irregularidades")

        print(f"\n📊 DETALHAMENTO FINANCEIRO:")
        print(f"   💰 Valor Total CT: R$ {resultado_pdd.get('valor_total_ct', 0):,.2f}")
        print(f"   🏠 Valor Total IPTU: R$ {resultado_pdd.get('valor_total_iptu', 0):,.2f}")
        print(f"   🔴 Valor Total Vencido: R$ {resultado_pdd.get('valor_total_vencido', 0):,.2f}")
        ...
        if saldo_atual > 0:
            ...
            if calculo_resultado.get("sucesso"):
                ...
                print(f"   💵 Novo Saldo: R$ {calculo_resultado['novo_saldo']:,.2f}")
                print(f"   📄 Detalhamento: {valores['detalhamento']}")
                print(f"   📆 Primeiro Vencimento: {valores['data_primeiro_vencimento']}")
                print(f"   📊 Quantidade Parcelas: {valores['quantidade_parcelas']}")
        ...
        if parcelas_desmarcar:
            ...
            for i, parcela in enumerate(parcelas_desmarcar[:3], 1):  # Mostrar primeiras 3
                print(f"   {i}. {parcela['documento']} - Venc: {parcela['data_vencimento']} - Motivo: {parcela['motivo']}")
        ...
        # Parcelas a vencer
        parcelas_a_vencer = df_titulo[df_titulo["Status da parcela"] == "A vencer"]
        print(f"   ⏳ Parcelas A Vencer: {len(parcelas_a_vencer)}")

        # Análise por indexador
        indexadores = df_titulo["Indexador"].value_counts()
        ...
        if len(parcelas_a_vencer) > 0:
            ...
            valores_a_receber = parcelas_a_vencer["Valor a receber"].dropna()
        ...
        print(f"🎯 Status Final: {'✅ PODE REPARCELAR' if resultado_pdd.get('pode_reparcelar') else '❌ NÃO PODE REPARCELAR'}")
...
async def teste_comparacao_planilhas_google_vs_csv():
    ...
    try:
        ...
        print(f"📋 Google Sheets - Sucesso: {'✅' if resultado_sheets.sucesso else '❌'}")
        ...
        print("\n🔍 COMPARAÇÃO:")
        print(f"   Google Sheets: {'✅ FUNCIONANDO' if resultado_sheets.sucesso else '❌ ERRO'}")
        print(f"   CSV Direto: {'✅ FUNCIONANDO' if resultado_csv else '❌ ERRO'}")
...
# Funções auxiliares dos testes originais (mantidas para compatibilidade)
async def teste_completo_original():
    """Teste completo original (Google Sheets)"""
    from rpa_analise_planilhas.teste_analise_planilhas import teste_completo
    return await teste_completo()
...
async def verificar_saude_sistema():
    ...
    from rpa_analise_planilhas.teste_analise_planilhas import verificar_saude_rpa
    return await verificar_saude_rpa()
