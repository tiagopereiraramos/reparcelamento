...
class ProcessadorRegrasNegocio:
    ...
    def processar_dados_cliente(self, df_planilha: pd.DataFrame, cliente: str,
        ...
        try:
            ...
            validacao_inadimplencia = self._aplicar_regra_inadimplencia(
                df_planilha)
            ...
            resultado_regras["regras_aplicadas"]["regra_1"] = self._regra_1_dia_vencimento(
                df_planilha)

            # REGRA 2: Cálculo primeiro vencimento
            resultado_regras["regras_aplicadas"]["regra_2"] = self._regra_2_primeiro_vencimento(
                df_planilha)

            # REGRA 3: Valor da parcela atual
            resultado_regras["regras_aplicadas"]["regra_3"] = self._regra_3_valor_parcela_atual(
                df_planilha)

            # REGRA 4: Verificação parcelas irregulares
            resultado_regras["regras_aplicadas"]["regra_4"] = self._regra_4_parcelas_irregulares(
                df_planilha)

            # REGRA 5: Quantidade parcelas a vencer
            resultado_regras["regras_aplicadas"]["regra_5"] = self._regra_5_parcelas_a_vencer(
                df_planilha)

            # REGRA 6: Quantidade parcelas vencidas CT
            resultado_regras["regras_aplicadas"]["regra_6"] = self._regra_6_parcelas_vencidas_ct(
                df_planilha)

            # REGRA 7: Pendências REC/FAT
            resultado_regras["regras_aplicadas"]["regra_7"] = self._regra_7_pendencias_rec_fat(
                df_planilha)
            ...
            resultado_consolidado = self._consolidar_resultados(
    ...
    def validar_cliente_inadimplencia(self, df_planilha: pd.DataFrame, cliente: str, numero_titulo: str) -> Dict[str, Any]:
        ...
        try:
            ...
            valores_financeiros = self._calcular_valores_financeiros(
                df_planilha)
            ...
            for _, row in parcelas_ct.iterrows():
                try:
                    ...
                    data_venc = pd.to_datetime(
                        row["Data vencimento"], errors='coerce')
                    ...
                    quitada = status in ["QUITADA",
                                         "LIQUIDADA", "PAGA", "BAIXADA"]
    ...
    def _regra_1_dia_vencimento(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            parcelas_a_vencer = df[
                df["Status da parcela"].str.upper().str.contains(
                    "VENCER|PENDENTE", na=False)
            ].copy()
        ...
        try:
            ...
            if tipo_reajuste.upper() == "ANUAL":
                ...
                primeiro_vencimento = date(
                    hoje.year, mes_base_reparcelamento, dia_vencimento)
                ...
                if dia_vencimento < dia_aniversario:
                    ...
                    if proximo_mes > 12:
                        ...
                        ano = hoje.year
                    primeiro_vencimento = date(
                        ano, proximo_mes, dia_vencimento)
        ...
        try:
            ...
            for _, row in parcelas_ct.iterrows():
                try:
                    ...
                    tipo_condicao = str(
                        row.get("Tipo condição", "")).strip().upper()
