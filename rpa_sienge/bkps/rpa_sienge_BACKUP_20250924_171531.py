...
class RPASienge(BaseRPA):
    ...
    async def _preencher_dados_relatorio_sienge(self, planilha, dados_financeiros: Dict[str, Any]):
        ...
        try:
            ...
            # ✅ CORREÇÃO: Definir inadimplência como "OK" se cliente for inadimplente
            pendencia_inadimplencia = "OK" if cliente_inadimplente else dados_validacao.get(
                "pendencias_sienge_inad", "")

            # ✅ CORREÇÃO CRÍTICA #1: Extrair tipo de reajuste da planilha ANTES de preenchimento
            ...
            try:
                ...
                # Buscar coluna "Tipo reajuste" ou "Tipo"
                coluna_tipo = None
                for j, col_nome in enumerate(cabecalho):
                    col_upper = str(col_nome).upper().strip()
                    if ('TIPO' in col_upper and 'REAJUSTE' in col_upper) or col_upper == 'TIPO':
                        coluna_tipo = j
                        self.log_progresso(
                            f"🔍 CORREÇÃO #1: Coluna 'Tipo reajuste' encontrada na coluna {j}: '{col_nome}'")
                        break

                # Se encontrou a coluna, buscar linha do contrato para extrair o valor
                if coluna_tipo is not None:
                    for i, linha in enumerate(todas_linhas[1:], start=2):
                        if len(linha) >= 7:  # Garantir que tem colunas suficientes
                            # Usar mesma lógica de busca do contrato (colunas 2 e 6)
                            ...
                            if (cliente_planilha == str(codigo_cliente) and
                                ...
                                if len(linha) > coluna_tipo:
                                    ...
                                    self.log_progresso(
                                        f"⚠️ CORREÇÃO #1: Linha do contrato não tem coluna tipo suficiente")
                                break
                else:
                    self.log_progresso(
                        f"⚠️ CORREÇÃO #1: Coluna 'Tipo reajuste' não encontrada na planilha, usando padrão: 'anual'")
            except Exception as e:
                self.log_progresso(
                    f"❌ CORREÇÃO #1: Erro ao extrair tipo de reajuste da planilha: {e}, usando padrão: 'anual'")
...
async def executar_sienge(
    ...
    def _calcular_primeiro_vencimento_pdd(self, dia_vencimento: int, tipo_reajuste: str, codigo_cliente: str, numero_titulo: str, planilha) -> str:
        """
        ...
        try:
            ...
            # Determinar mês base de reparcelamento (próximo mês conforme regras atualizadas)
            hoje = date.today()
            mes_base = hoje.month
            ano_base = hoje.year

            # Calcular mês de reparcelamento (próximo mês)
            mes_reparcelamento = mes_base + 1
            ano_reparcelamento = ano_base
            if mes_reparcelamento > 12:
                mes_reparcelamento = 1
                ano_reparcelamento += 1

            self.log_progresso(
                f"🔍 CORREÇÃO #2/#3: Mês reparcelamento: {mes_reparcelamento:02d}/{ano_reparcelamento}")
            ...
            if 'anivers' in tipo_lower:
                ...
                if data_assinatura:
                    ...
                    # Fallback para regra anual se não conseguir encontrar data de assinatura
                    self.log_progresso(
                        f"⚠️ CORREÇÃO #2/#3: Data assinatura não encontrada, aplicando regra ANUAL")
                    primeiro_vencimento = datetime(
                        ano_reparcelamento, mes_reparcelamento, dia_vencimento)
            else:
                # ✅ REGRA ANUAL PDD
            ...
            fallback = proximo_mes.replace(
                day=dia_vencimento).strftime("%Y-%m-%d")
            self.log_progresso(
                f"🔄 CORREÇÃO #2/#3: Usando fallback: {fallback}")
            return fallback
