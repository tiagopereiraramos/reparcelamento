...
class JSONRPAFramework:
    ...
    def __init__(self,
        ...
        self.auto_index_fields = auto_index_fields or [
            'status', 'codigo_cliente', 'empresa']
    ...
    def _save_data(self, data: List[Dict[str, Any]]) -> None:
        ...
        with self._cache_lock:
            self._data_cache = deepcopy(data)
            self._cache_dirty = False
        self._build_indexes()
        self.compact_wal()

    def insert(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """Insere um novo registro, retornando-o com _id gerado"""
        record = deepcopy(record)
        record_id = record.get('_id') or self._generate_id()
        record['_id'] = record_id
        record['_created_at'] = record.get(
            '_created_at', self._get_timestamp())
        record['_updated_at'] = self._get_timestamp()
                ...
                for field in self.auto_index_fields:
                    if field in record:
                        self.index_manager.update_index(
                            field, None, record[field], record_id)
                if '_id' in record:
                    self.index_manager.update_index(
                        '_id', None, record['_id'], record_id)
    ...
    def update(self, query: Dict[str, Any], update_data: Dict[str, Any], multi: bool = False) -> int:
        ...
        if '_created_at' in update_data:
            update_data.pop('_created_at')
                ...
                if QueryBuilder.match_record(record, query):
                    ...
                    for field in self.auto_index_fields:
                        ...
                        if old_value != new_value:
                            self.index_manager.update_index(
                                field, old_value, new_value, record_id)
            ...
            for record in data:
                if QueryBuilder.match_record(record, query):
                    ...
                    for field in self.auto_index_fields:
                        if field in record:
                            self.index_manager.update_index(
                                field, record[field], None, record_id)
        ...
        with self.main_lock:
            ...
            for i, record in enumerate(data):
                if record.get('_id') == record_id:
                    ...
                    update_payload.setdefault('Titulo', record.get('Titulo'))
                    update_payload.setdefault('Cliente', record.get('Cliente'))
                    update_payload.setdefault(
                        'Código Cliente', record.get('Código Cliente'))
                    data[i].update(update_payload)

                    # Atualiza índice de status
                    if 'status' in self.auto_index_fields:
                        self.index_manager.update_index(
                            'status', old_status, status, record_id)
                        ...
                        for i, record in enumerate(data):
                            if record.get('_id') == entry.record_id:
                                if '_id' not in record:
                                    record['_id'] = entry.record_id
                                data[i].update(entry.data)
                                break
                    elif entry.operation == 'DELETE':
                        data = [r for r in data if r.get(
                            '_id') != entry.record_id]
    ...
    def _get_cached_data(self) -> List[Dict[str, Any]]:
        ...
        with self._cache_lock:
            if self._data_cache is None or self._cache_dirty:
                self._refresh_cache()
...
if __name__ == "__main__":
    ...
    # Atualiza status
    framework.set_status(record_id, "processando", {
