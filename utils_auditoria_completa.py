...
class ConsultorAuditoriaCompleta:
    ...
    async def obter_execucao_completa(self, id_execucao: str) -> Dict[str, Any]:
        ...
        try:
            # Busca consolidado
            arquivo_consolidado = self.pasta_auditoria / f"CONSOLIDADO_{id_execucao}.json"
        ...
        try:
            ...
            for execucao in execucoes:
                ...
                if timestamp and datetime.fromisoformat(timestamp.replace('Z', '')) >= data_limite:
                    ...
                    stats["execucoes_por_rpa"][nome_rpa] = stats["execucoes_por_rpa"].get(nome_rpa, 0) + 1
            ...
            if execucoes_periodo:
                sucessos = sum(1 for e in execucoes_periodo if e.get("sucesso", False))
                stats["taxa_sucesso_geral"] = round((sucessos / len(execucoes_periodo)) * 100, 2)
    ...
    async def gerar_relatorio_completo(self, numero_titulo: str = None, 
        ...
        if numero_titulo:
            ...
            for rpa in ["RPA_Coleta_Indices", "RPA_Analise_Planilhas", "RPA_Sienge"]:
                ...
                relatorio["ultimas_execucoes"][rpa] = execucoes[:5]  # Últimas 5
    ...
    async def _buscar_mongodb_por_filtros(self, filtros: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            def _search():
                ...
                cursor = mongodb_manager.database.execucoes_consolidadas_rpa.find(query)
                return list(cursor)
        ...
        if isinstance(documento, dict):
            ...
            for key, value in documento.items():
                if hasattr(value, 'isoformat'):  # datetime
                    ...
                    documento[key] = [self._limpar_objectids(item) if isinstance(item, dict) else item for item in value]
