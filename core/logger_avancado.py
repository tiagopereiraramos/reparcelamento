...
class LoggerAvancado:
    ...
    def __init__(self,
                 ...
                 webhook_ativo: Optional[bool] = None):
        """
        ...
        self.webhook_url = webhook_url or os.getenv(
            'WEBHOOK_LOGS_URL', 'http://177.39.21.61:3009/logs')
        if webhook_ativo is None:
            env_flag = os.getenv('WEBHOOK_LOGS_ATIVO', 'false').lower()
            webhook_ativo = env_flag in {"1", "true", "yes", "on"}
        self.webhook_ativo = webhook_ativo
    ...
    def _configurar_logger(self) -> logging.Logger:
        ...
        try:
            ...
            log_dir = Path("outputs") / now.strftime('%Y') / \
    ...
    def register_log(self,
        ...
        try:
            ...
            if self.webhook_ativo and enviar_webhook:
                log_entry = {
    ...
    def _enviar_webhook(self, log_entry: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                self.logger.debug(
                    "Log enviado via webhook",
                    extra={"nivel_webhook": log_entry['nivel']}
                )
                return True
            else:
                self.logger.warning(
            ...
            self.logger.warning("Webhook indisponível", exc_info=e)
            return False
