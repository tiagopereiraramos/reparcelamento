...
class RastreamentoUnificado:
    ...
    def __init__(self, nome_rpa: str):
        ...
        self.arquivo_execucao = self.pasta_auditoria / f"{self.id_execucao}.json"
    ...
    async def finalizar_rastreamento(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Rastreamento finalizado: {self.id_execucao} ({len(self.passos_execucao)} passos)")
    ...
    async def _salvar_documento_consolidado(self, documento_final: Dict[str, Any]):
        ...
        # JSON (obrigatório)
        arquivo_consolidado = self.pasta_auditoria / f"CONSOLIDADO_{self.id_execucao}.json"
        try:
            with open(arquivo_consolidado, 'w', encoding='utf-8') as f:
                json.dump(documento_final, f, indent=2, ensure_ascii=False, default=str)
        except Exception as e:
